...
**Web client deviation: PBKDF2-HMAC-SHA256 instead of Argon2id.** The HTML/JS web client wraps the private key using PBKDF2-HMAC-SHA256 (600 000 iterations, 16-byte random salt, 32-byte output → AES-256-GCM wrap) rather than Argon2id. This is a deliberate platform constraint, not a design choice: the Web Crypto API (`SubtleCrypto`) exposes PBKDF2 as its only password-based KDF; Argon2id is not available in any browser's built-in cryptography API and cannot be safely implemented in pure JavaScript without a constant-time WASM build that would add significant bundle complexity. PBKDF2-HMAC-SHA256 at 600 000 iterations is the OWASP-recommended minimum for PBKDF2 (OWASP Password Storage Cheat Sheet, 2024), tuned to a ~0.5 s cost on a modern machine. The security gap relative to Argon2id is real: PBKDF2 is not memory-hard and is therefore more susceptible to GPU/ASIC parallelism on a stolen key blob. This is accepted for the web client because (a) private keys are stored in `localStorage` which is already a lower-security boundary than the C++ client's filesystem-protected `keys.bin`, and (b) the web client is the optional second client — the C++ client remains the primary, and it does use Argon2id at `SENSITIVE` parameters. This limitation is documented here so that any future web client revision can substitute a WASM-compiled Argon2id drop-in when browser support matures.
