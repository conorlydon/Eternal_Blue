...
- **`app.js`** — all logic as a single ES module; no bundler, no dependencies beyond what is imported from `esm.sh`

The only external dependencies are loaded at runtime via `esm.sh`:

```js
import { CipherSuite, DhkemX25519HkdfSha256, HkdfSha256 } from 'https://esm.sh/@hpke/core@1.7.1'
import { Chacha20Poly1305 } from 'https://esm.sh/@hpke/chacha20poly1305@1.7.1'
```
...
- The Content-Security-Policy served by the backend allows `script-src 'self' https://esm.sh` and blocks inline scripts.
