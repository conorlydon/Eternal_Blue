...
| `frontend/lib/` | Locally-bundled vendor code (`hpke.bundle.js`), served from `'self'` |

Neither page uses a framework — both are plain ES modules loaded by the browser. The only build step is a one-off esbuild bundle of the HPKE library for the app (`npm run build:hpke`); the committed bundle is served as-is and there is no runtime bundler.
...
- **`app.js`** — all logic as a single ES module; no framework

The HPKE library is **bundled and served locally** — nothing is fetched from an external CDN at runtime:

```js
import { CipherSuite, DhkemX25519HkdfSha256, HkdfSha256, Chacha20Poly1305 } from '/lib/hpke.bundle.js'
```

`frontend/lib/hpke.bundle.js` is generated by esbuild from `@hpke/core` and `@hpke/chacha20poly1305` (pinned to `1.7.1`, installed as dev dependencies in `backend/`). The bundle re-exports the four symbols the app needs. Regenerate it after bumping the library version with:

```bash
cd backend && npm run build:hpke
```

The `build:hpke` script bundles `backend/build/hpke-entry.js` into `frontend/lib/hpke.bundle.js`. Serving HPKE from `'self'` keeps the cryptographic dependency under our control (no CDN trust, no third-party availability dependency) and lets the Content-Security-Policy drop the esm.sh allowances entirely.
...
- The Content-Security-Policy served by the backend restricts `script-src` and `connect-src` to `'self'` and blocks inline scripts. With HPKE bundled locally, the app loads no external scripts, so the previous `https://esm.sh` allowances have been removed.
